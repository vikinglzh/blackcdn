    ...
    if( danmuon > 0){
	var dp = new yzmplayer({autoplay: autoplay,hotkey: true,element: document.getElementById('player'),theme: color,logo: logo,video: {url: vodurl,pic: vodpic,type: 'auto',},danmaku: {id: dmid,api: dmapi,user: user,}});
	}else{$('body').addClass("danmu-off");
	var dp = new yzmplayer({autoplay: autoplay,hotkey: true,element: document.getElementById('player'),theme: color,logo: logo,video: {url: vodurl,pic: vodpic,type: 'auto',},});}
    // 弹出框
...
function video_con_play() {
		...
		timer = setInterval(function(){
         ...
         $(".conplaying").on("click", function () {clearTimeout(timer);$("#laoding-pic,#loading-box").remove();});
